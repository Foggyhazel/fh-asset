...
def alert(parent, title, text, buttons=QMessageBox.Ok):
...
def warnConfirm(parent, title, text):
    msg = QMessageBox(parent)
    msg.setWindowTitle(title)
    msg.setText(text)
...
class AssetItem(QStyledItemDelegate):
    def paint(self, painter, option, index):
        return super(AssetItem, self).paint(painter, option, index)


class AssetTreeView(QTreeView):
    def __init__(self, parent=None):
        super(AssetTreeView, self).__init__(parent)
    ...
    def setDisableEdit(self, disallow_path):
    ...
    def edit(self, *args):
        ...
        if path in self._disableEdit:
            return False
        return super(AssetTreeView, self).edit(*args)

    def getCurrentFileModelIndex(self):
...
class AssetBrowser(QWidget):
    ...
    def updateFileView(self, file_model_index):
    ...
    def updateInfoView(self, asset_path):
        info = asset.getAsset(asset_path)
        # houhelper.loadAsset(info, None)
        self.infoWidget.setAsset(info)

    def handleTreeSelectionChanged(self, selected, deselected):
        self.updateFileView(self.treeWidget.getCurrentFileModelIndex())

    def handleListSelectionChanged(self, selected, deselected):
        index = self.listWidget.currentIndex()
        path = self.model.filePath(index)
        self.updateInfoView(path)

    def handleListDoubleClicked(self, index):
        model = self.model
    ...
    def dropEvent(self, event):
        ...
        items = item_data.data().decode('utf-8').split('\t')
        payload = {
...
# endregion
    ...
    def beginCreateAsset(self, payload, path_to_create_in):
        def onSavePress(data):
    ...
    def beginEditAsset(self, payload):
...
class EditAssetWindow(QWidget, Ui_EditAsset):
    Mode = enum(New=0, Edit=1)

    def __init__(self, parent=None, mode=Mode.New, asset_type=None, onSavePress=None):
    ...
    def resizeEvent(self, event):
        ...
        return super(EditAssetWindow, self).resizeEvent(event)
    ...
    def setIntialData(self, assetObj, version=None, asset_def=None):
    ...
    def closeEvent(self, event):
        ...
        super(EditAssetWindow, self).closeEvent(event)
        self.setParent(None)
    ...
    def setPreviewImage(self, filepath):
...
class AssetInfoWidget(QWidget, Ui_AssetInfo):
    ...
    def setAsset(self, assetObj, use_latest_version=False):
        if assetObj is None:
            self.clear()
        else:
